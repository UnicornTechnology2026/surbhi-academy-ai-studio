...
import { Star, Sparkles, Quote, ArrowRight, Award } from "lucide-react";
...
export const TestimonialsPage: React.FC<TestimonialsPageProps> = ({
  ...
  const filtered = activeTestimonials.filter((t) => {
    ...
    if (filter === "Student")
      return (
        t.relation?.toLowerCase().includes("student") ||
        t.role?.toLowerCase().includes("student")
      );
    if (filter === "Parent")
      return (
        t.relation?.toLowerCase().includes("parent") ||
        t.role?.toLowerCase().includes("parent")
      );
    return true;
  });
  ...
  return (
    <div className="space-y-16 py-8">
      {/* 1. Hero Header */}
      <section className="relative overflow-hidden rounded-3xl bg-linear-to-br from-slate-900 via-[#0F172A] to-slate-950 p-8 sm:p-14 text-white border border-slate-800 shadow-2xl">
        ...
        <div className="max-w-3xl space-y-4 relative z-10">
          ...
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
            Read firsthand experiences from students who elevated their board
            examination ranks and parents who trusted {siteSettings.name} with
            their children’s academic foundation.
          </p>
        </div>
      </section>
      ...
      <section className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div className="text-sm text-slate-300 font-semibold">
          Showing <strong className="text-amber-400">{filtered.length}</strong>{" "}
          Genuine Reviews
        </div>
      ...
      {/* 6. Call to Action */}
      <section className="bg-linear-to-r from-amber-500 via-amber-400 to-amber-500 text-slate-950 rounded-3xl p-8 sm:p-12 text-center space-y-6 shadow-2xl">
        ...
        <p className="text-xs sm:text-sm text-slate-900 max-w-xl mx-auto font-medium">
          Experience our teaching methodology first-hand. Book a 2-day
          complimentary classroom demo session at our academy.
        </p>
